	...
	LOD 0
		...
		Pass
		{
			...
			fixed4 frag (v2f i , half ase_vface : VFACE) : SV_Target
			{
				...
				UNITY_BRANCH 
				if( _NorDist == 1.0 )
				ifLocalVar3495 = ( distance( WorldPosition , ( (float)0 == 0.0 ? localCenterEye1_g325 : _WorldSpaceCameraPos ) ) * 2.0 * _NorDist );
				...
				float3 localCenterEye1_g326 = CenterEye1_g326();
				float3 temp_output_4_0_g326 = ( localCenterEye1_g326 - WorldPosition );
				float3 normalizeResult5_g326 = normalize( temp_output_4_0_g326 );
				float3 lerpResult2635 = lerp( appendResult248 , normalizeResult5_g326 , float3( 0.3333,0.3333,0.3333 ));
				...
				float3 localCenterEye1_g350 = CenterEye1_g350();
				float3 ifLocalVar50_g349 = 0;
				UNITY_BRANCH 
				if( (int)_StereoFix == 1.0 )
				ifLocalVar50_g349 = localCenterEye1_g350;
				else if( (int)_StereoFix < 1.0 )
				ifLocalVar50_g349 = _WorldSpaceCameraPos;
				float3 worldToViewDir27_g349 = normalize( mul( UNITY_MATRIX_V, float4( ( ifLocalVar50_g349 - WorldPosition ), 0 ) ).xyz );
				...
				float3 worldToViewDir13_g349 = normalize( mul( UNITY_MATRIX_V, float4( NormalMatCap3089, 0 ) ).xyz );
				float mulTime2208 = _Time.y * _MatcapSpin;
				float cos2210 = cos( mulTime2208 );
				float sin2210 = sin( mulTime2208 );
				float2 rotator2210 = mul( (float2( 0,0 ) + ((BlendNormals( ( float3( -1,-1,1 ) * worldToViewDir27_g349 ) , worldToViewDir13_g349 )).xy - float2( -1,-1 )) * (float2( 1,1 ) - float2( 0,0 )) / (float2( 1,1 ) - float2( -1,-1 ))) - float2( 0.5,0.5 ) , float2x2( cos2210 , -sin2210 , sin2210 , cos2210 )) + float2( 0.5,0.5 );
				float3 localCenterEye1_g348 = CenterEye1_g348();
				float3 ifLocalVar32_g347 = 0;
				UNITY_BRANCH 
				if( (int)_StereoFix == 1.0 )
				ifLocalVar32_g347 = localCenterEye1_g348;
				else if( (int)_StereoFix < 1.0 )
				ifLocalVar32_g347 = _WorldSpaceCameraPos;
				float3 normalizeResult19_g347 = normalize( ( ifLocalVar32_g347 - WorldPosition ) );
				half3 _worldUp = half3(0,1,0);
				float3 normalizeResult8_g347 = normalize( cross( normalizeResult19_g347 , _worldUp ) );
				float3 temp_output_21_0_g347 = NormalMatCap3089;
				float dotResult10_g347 = dot( normalizeResult8_g347 , temp_output_21_0_g347 );
				float dotResult2_g347 = dot( _worldUp , normalizeResult19_g347 );
				float3 normalizeResult7_g347 = normalize( ( _worldUp - ( dotResult2_g347 * normalizeResult19_g347 ) ) );
				float dotResult9_g347 = dot( normalizeResult7_g347 , temp_output_21_0_g347 );
				float2 appendResult11_g347 = (float2(dotResult10_g347 , dotResult9_g347));
				float cos2207 = cos( mulTime2208 );
				float sin2207 = sin( mulTime2208 );
				float2 rotator2207 = mul( ( ( appendResult11_g347 * float2( 0.5,0.5 ) ) + float2( 0.5,0.5 ) ) - float2( 0.5,0.5 ) , float2x2( cos2207 , -sin2207 , sin2207 , cos2207 )) + float2( 0.5,0.5 );
				...
				ifLocalVar3493 = normalizeResult5_g327;
				...
				float3 ifLocalVar32_g341 = 0;
				UNITY_BRANCH 
				if( (int)_StereoFixSpecCap == 1.0 )
				ifLocalVar32_g341 = localCenterEye1_g342;
				else if( (int)_StereoFixSpecCap < 1.0 )
				ifLocalVar32_g341 = _WorldSpaceCameraPos;
				float3 normalizeResult19_g341 = normalize( ( ifLocalVar32_g341 - WorldPosition ) );
				float3 normalizeResult8_g341 = normalize( cross( normalizeResult19_g341 , _worldUp ) );
				float3 temp_output_21_0_g341 = NormalSpecular965;
				float dotResult10_g341 = dot( normalizeResult8_g341 , temp_output_21_0_g341 );
				float dotResult2_g341 = dot( _worldUp , normalizeResult19_g341 );
				float3 normalizeResult7_g341 = normalize( ( _worldUp - ( dotResult2_g341 * normalizeResult19_g341 ) ) );
				float dotResult9_g341 = dot( normalizeResult7_g341 , temp_output_21_0_g341 );
				float2 appendResult11_g341 = (float2(dotResult10_g341 , dotResult9_g341));
				...
				ifLocalVar3491 = tex2D( _SpecularMatcap, ( ( appendResult11_g341 * float2( 0.5,0.5 ) ) + float2( 0.5,0.5 ) ) );
				...
				float3 localCenterEye1_g330 = CenterEye1_g330();
				float clampResult2503 = clamp( distance( WorldPosition , ( (float)0 == 0.0 ? localCenterEye1_g330 : _WorldSpaceCameraPos ) ) , 0.0 , 100.0 );
				...
				float3 localCenterEye1_g334 = CenterEye1_g334();
				float4 appendResult76_g333 = (float4(( WorldPosition - localCenterEye1_g334 ) , 0.0));
				float4 temp_output_77_0_g333 = mul( UNITY_MATRIX_V, appendResult76_g333 );
				float2 break83_g333 = ( (temp_output_77_0_g333).xy / (temp_output_77_0_g333).z );
				float2 appendResult86_g333 = (float2(( ( _ScreenParams.z / _ScreenParams.w ) * break83_g333.x ) , ( break83_g333.y * 2.0 )));
				float2 appendResult101_g333 = (float2(1.0 , 1.0));
				float2 appendResult102_g333 = (float2(0.0 , 0.0));
				...
				ifLocalVar3487 = ( ( ( ( -appendResult86_g333 * float2( 1,0.5 ) ) + float2( 0.5,0.5 ) ) * appendResult101_g333 ) + appendResult102_g333 );
				...
				float3 smoothstepResult901 = smoothstep( temp_cast_20 , temp_cast_21 , ( desaturateVar1704 * desaturateVar1705 * ( 1.0 - pow( ( distance( WorldPosition , localCenterEye1_g328 ) / 10000.0 ) , 0.5 ) ) ));
...
/*ASEBEGIN
Version=18814
-1635;27;1486;818;2354.467;5296.868;1.052954;True;False
...
Node;AmplifyShaderEditor.FunctionNode;3511;-3332.646,-1277.474;Inherit;False;VR_Center_Eye;-1;;326;c8866f5c3b9339443aaf9087a76ba9d4;2,13,1,14,0;0;2;FLOAT3;0;FLOAT3;2
...
Node;AmplifyShaderEditor.FunctionNode;3512;-1783.858,-2540.273;Inherit;False;VR_Center_Eye;-1;;327;c8866f5c3b9339443aaf9087a76ba9d4;2,13,1,14,0;0;2;FLOAT3;0;FLOAT3;2
...
Node;AmplifyShaderEditor.FunctionNode;3514;-2143.525,-3346.59;Inherit;False;View Distance;-1;;329;2a6ef75a50a2bd147a81c448d8de0d4d;1,8,0;1;9;INT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.FunctionNode;3520;-1572.417,-1950.33;Inherit;False;VR MatCap;-1;;341;70ea9bc890dab6b42ba77ce7cb65ac19;0;2;21;FLOAT3;0,0,0;False;27;INT;0;False;1;FLOAT2;0
...
Node;AmplifyShaderEditor.FunctionNode;3516;-3645.763,-4266.91;Inherit;False;VR_ScreenSpaceUV;-1;;333;83887303006e6e7459722034b3b9f189;0;4;97;FLOAT;1;False;98;FLOAT;1;False;99;FLOAT;0;False;100;FLOAT;0;False;1;FLOAT2;0
...
Node;AmplifyShaderEditor.FunctionNode;3523;679.5027,1427.81;Inherit;False;VR MatCap;-1;;347;70ea9bc890dab6b42ba77ce7cb65ac19;0;2;21;FLOAT3;0,0,0;False;27;INT;0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.FunctionNode;3524;675.3692,1207.173;Inherit;False;Matcap (legacy);-1;;349;0c9402088b71ec54f9209482380424ee;1,37,1;2;9;FLOAT3;0,0,0;False;45;INT;0;False;1;FLOAT2;0
...
Node;AmplifyShaderEditor.FunctionNode;3513;-1771.232,-4986.866;Inherit;False;VR_Center_Eye;-1;;328;c8866f5c3b9339443aaf9087a76ba9d4;2,13,1,14,0;0;2;FLOAT3;0;FLOAT3;2
...
Node;AmplifyShaderEditor.FunctionNode;3526;-264.6091,-3048.01;Inherit;False;Blinn-Phong Half Vector VR;-1;;352;a14416ee163e75d4f83c3889a3f25b2f;0;0;1;FLOAT3;0
...
Node;AmplifyShaderEditor.FunctionNode;3522;-1767.401,-1224.196;Inherit;False;Matcap (legacy);-1;;345;0c9402088b71ec54f9209482380424ee;1,37,1;2;9;FLOAT3;0,0,0;False;45;INT;0;False;1;FLOAT2;0
...
ASEEND*/
//CHKSM=2CEF7CDE67504EF1B8C10C51F283DD83B1DA51B2
